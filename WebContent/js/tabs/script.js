(function($){
	...
	$('#containertabs').css('height', $('#contentstabs div').eq(0).height() + 20);
	...
	var tabs = $('#tabs li').each(function(i){
		...
		$(this).attr('tabid', i).click(function(){
			...
			if (trigger == false){
				...
				// 使內容移動一定間距
				$('#contentstabs').animate({left: '-='+gap}, 535);
				// 使內容高度符合所選頁籤內容的高度（所選頁籤內容高度 + 上下padding），動畫全部結束後，使trigger值返回false
				$('#containertabs').animate({height: $('#contentstabs').children().eq(_i).height() + 20}, 500, function(){
					trigger = false;
				});
			}
		});
